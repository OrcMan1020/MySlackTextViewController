...
- (void)viewDidLoad
{
    ...
    [self addObserver:self forKeyPath:@"keyboardHC.constant" options:NSKeyValueObservingOptionNew context:nil];
}
...
- (SLKNewInputAccessoryView *)textInputbar
{
    if (!_textInputbar) {
        NSArray* nibView = [[NSBundle mainBundle] loadNibNamed:@"SLKNewInputAccessoryView" owner:nil options:nil];
        _textInputbar = [nibView firstObject];

        
        _textInputbar.translatesAutoresizingMaskIntoConstraints = NO;

        [self addActionsForTextInputBar];
        ...
        _textInputbar.textView.placeholder = @"Message to Sai";
    }
    return _textInputbar;
}

-(void)addActionsForTextInputBar
{
    [self.textInputbar.sendButton addTarget:self action:@selector(didPressSendButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.saveButton addTarget:self action:@selector(didCommitTextEditing:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.cancelButton addTarget:self action:@selector(didCancelTextEditing:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.atMentionButton addTarget:self action:@selector(didPressAtMentionButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.slashCommandButton addTarget:self action:@selector(didPressSlashCommandButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.filesButton addTarget:self action:@selector(didPressFilesButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.photosButton addTarget:self action:@selector(didPressPhotosButton:) forControlEvents:UIControlEventTouchUpInside];
    [self.textInputbar.formButton addTarget:self action:@selector(didPressFormButton:) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)textDidUpdate:(BOOL)animated
{
    ...
    if (!self.textInputbar.inputComposerExpanded) {
        CGFloat inputbarHeight = _textInputbar.appropriateHeight;
        _textInputbar.sendButton.enabled = [self canPressSendButton];
...
-(void)didPressSendButton:(id)sender
{
    ...
    if (self.textInputbar.inputComposerExpanded) {
        [self didPressFormButton:nil];
    }
    
}
-(void)didPressFormButton:(id)sender
{
    self.textInputbar.inputComposerExpanded = !self.textInputbar.inputComposerExpanded;
//    self.textInputbar.roundCorner.hidden = !self.textInputbar.inputComposerExpanded;
...
//        self.textView.textContainerInset = UIEdgeInsetsMake(10, 11, 10, 41);
    ...
    if (!self.textInputbar.inputComposerExpanded) {
        ...
        [self updateTextInputbarHCWhenInputComposerExpanded];
    }
...
-(void)updateTextInputbarHCWhenInputComposerExpanded
{
    if (!self.textInputbar.inputComposerExpanded) {
        return;
    }
    ...
    CGFloat inputbarHeight = self.view.bounds.size.height - self.keyboardHC.constant - 32.f;
    CGFloat inputBarHeightDelta = inputbarHeight - self.textInputbarHC.constant;
    self.textInputbarHC.constant = inputbarHeight;
    ...
    [self.view slk_animateLayoutIfNeededWithBounce:YES
                                           options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionLayoutSubviews|UIViewAnimationOptionBeginFromCurrentState
                                        animations:^{
                                            if (!self.isInverted) {
                                                self.scrollViewProxy.contentOffset = newOffset;
...
-(void)didPressAtMentionButton:(SLKInputSegmentedSubcontrol*)sender
{
    NSMutableAttributedString* attrString = [self.textInputbar.textView.attributedText mutableCopy];
    if (!attrString) attrString = [NSMutableAttributedString new];
    [attrString appendAttributedString:[self.textInputbar.textView slk_defaultAttributedStringForText:@"@"]];
    self.textInputbar.textView.attributedText = attrString;
...
- (void)editAttributedText:(NSAttributedString *)attributedText
{
    ...
    _textInputbar.inputType = SLKInputModeEdit;
}
...
- (void)didCommitTextEditing:(id)sender
{
    ...
    _textInputbar.inputType = SLKInputTypeInput;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    else if([keyPath isEqualToString:@"keyboardHC.constant"]){
        [self updateTextInputbarHCWhenInputComposerExpanded];
    }
    else {
...
- (void)dealloc
{
    ...
    [self removeObserver:self forKeyPath:@"keyboardHC.constant"];
